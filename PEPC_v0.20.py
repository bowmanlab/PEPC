...
select = string.replace(select, ':', '_')
select = string.replace(select, ',', '_')
...
if exp_seq + '.blast' not in os.listdir('.'):
    ...
    blast = subprocess.Popen('blastp -num_alignments 1000 -evalue 1e-5 -query \"'+exp_seq+'.clean.fasta\" -db ' + db + ' -outfmt 0 -out \"' + exp_seq + '.blast\"', shell = True)
    blast.communicate()
...
for start in range(starts):
    s = s + 1
    log_out = open(exp_seq + '_' + str(s) + '_' + str(events) + '_' + str(required[4]) + '_' + select + '_' + '.log','w')
    fasta_out = open(exp_seq + '_' + str(s) + '_' + str(events) + '_' + str(required[4]) + '_' + select + '_' + '.fasta','w')
    aa_seq_str = aa_seq_str_i
    print >> fasta_out, '>'+exp_seq+'_'+str(s)+'_'+str(0)+'\n'+aa_seq_str
    ...
    for event in range(events):
        ...
        if keep == True:
            ...
            print >> fasta_out, '>'+exp_seq+'_'+str(s)+'_'+str(n)+'\n'+aa_seq_str 
            ...
            if roll[0] == r_dice:
                ...
                print >> log_out, s, n, m, rm, rp + 1, rr, cr, 'mutate on dice roll:', param_str
                pos_w_mut.append(i)
                aa_seq = new_aa_seq
                aa_seq_str = ''.join(aa_seq)
                ...
                print >> log_out, s, n, m, rm, rp + 1, rr, cr, 'none:', param_str
    ...
    with open(exp_seq + '_' + str(s) + '_' + str(events) + '_' + str(required[4]) + '_' + select + '_' + '.fasta', 'r') as fasta_hist:
    ...
    mutation_tally = open(exp_seq + '_' + str(s) + '_' + str(events) + '_' + str(required[4]) + '_' + select + '_' + '_tally.txt','w')
    for key in position_dict.keys():
        tally = len(position_dict[key])
        print >> mutation_tally, str(key)+'\t'+structure[key]+'\t'+str(tally)+'\t'+position_dict[key]
    mutation_tally.close()
    
print 'mutations =', m, 'random mutations =', rm, 'events =', events * starts
    
with open(exp_seq + '_' + str(starts) + '_' + str(events) + '_' + str(required[4]) + '_' + select + '_' + '.txt','w') as param_file:
    print >> param_file, 'run' + '\t' + 'generation' + '\t' + 'flex' + '\t' + 'gravy' + '\t' + 'iso' + '\t' + 'arom' + '\t' + 'ai'    
    nested_csv(param_file, param_output)
    
with open(exp_seq + '_coil_' + '_' +  str(starts) + '_' + str(events) + '_' + str(required[4]) + '_' + select + '_' + '.txt','w') as param_file:
    print >> param_file, 'run' + '\t' + 'generation' + '\t' + 'flex' + '\t' + 'gravy' + '\t' + 'iso' + '\t' + 'arom' + '\t' + 'ai'    
    nested_csv(param_file, coil_param_output)
    
with open(exp_seq + '_beta_' + '_' + str(starts) + '_' + str(events) + '_' + str(required[4]) + '_' + select + '_' + '.txt','w') as param_file:
    print >> param_file, 'run' + '\t' + 'generation' + '\t' + 'flex' + '\t' + 'gravy' + '\t' + 'iso' + '\t' + 'arom' + '\t' + 'ai'    
    nested_csv(param_file, beta_param_output)
    
with open(exp_seq + '_helix_' + '_' + str(starts) + '_' + str(events) + '_' + str(required[4]) + '_' + select + '_' + '.txt','w') as param_file:
    print >> param_file, 'run' + '\t' + 'generation' + '\t' + 'flex' + '\t' + 'gravy' + '\t' + 'iso' + '\t' + 'arom' + '\t' + 'ai'    
    nested_csv(param_file, helix_param_output)
    
with open(exp_seq + '_posit_var.txt', 'w') as var_out:
    for key in posit_var.keys():
        print >> var_out, key, posit_var[key]
